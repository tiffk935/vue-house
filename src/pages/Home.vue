<template>
  ...
  <div v-bind:class="{
    ...
    <img class="w-32" src="@/assets/loading_w.gif" alt="漫活時代2" srcset="">
  </div>
  <!--loading end-->
  <Nav v-if="config.showNav" />
  <div class="home bg-white overflow-hidden font-['Noto_Sans_TC']">
    <h1 style="display:none;">漫活時代2</h1>
    <S1 />
    <S2 />
    <S3 />
    <S4 />
...
onMounted(() => {
  window.onload = function () {
    ...
    setTimeout(() => {
      let status = 0;
      let hideHand = false;
      document.querySelector('.s2 .map').scrollLeft = document.querySelector('.s2 .map .map1').clientWidth * 0.53 - window.innerWidth/2;
